...
| `パイプが無ければ即座に返る` | 同名 | 移植 (フェーズ 7)。書き手側なので `conductor-mcp/src/refresh_signal.rs` |
| `読み手がいなければ即座に返る` | 同名 | 同上 |
| `普通のファイルには書き込まない` | 同名 | 同上 |

移植 15 / 新規 6 (統合テスト) / 削除 4 (core へ移設済みで重複)。書き手側の 3 本は
旧 `src/` を消すフェーズ 7 で conductor-mcp へ移した。
