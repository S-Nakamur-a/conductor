...
旧テスト 19 本 (下表) → 新テスト 37 本 (移植 14 / 削除 5)
...
| 旧テスト名 | 扱い | 新テスト名 / 削除理由 |
|---|---|---|
| ガターとバッジのクリックは必ずコメントを始める | 削除 | 新はガターを押しても作成に飛ばない。作成は `c` だけの入口にした (旧はマウスとキーボードで別の UI が開く 2 実装だった) |
...
1. **コメントの作成口が 1 つ。** 旧は `c` (アンカー方式) とガタークリック
   (`"file:line "` を文字列で前置きする方式) の 2 実装があり、マウスとキーボードで
   違う UI が開いた。新は `c` だけで、ガターの印はスレッドの開閉に使う
