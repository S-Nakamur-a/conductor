...
/// The kind of permission prompt detected on the PTY screen.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum PermissionPromptKind {
    /// Numbered selector (e.g. "1. Yes", "2. Yes, always allow …", "3. No").
    NumberedSelector,
    /// Simple [Y/n] or [y/N] prompt.
    YesNoPrompt,
}

/// Extracted permission choices from the PTY screen.
#[derive(Debug, Clone)]
pub struct PermissionChoices {
    /// The kind of prompt detected.
    pub kind: PermissionPromptKind,
    /// Available options as (0-based index, display text) pairs.
...
impl PtyManager {
    ...
    /// Extract the available permission choices from the PTY screen.
    ///
    /// Returns `None` if no recognisable permission prompt is visible.
    pub fn extract_permission_choices(&self, idx: usize) -> Option<PermissionChoices> {
        let session = self.sessions.get(idx)?;
        let parser = session.screen.lock().unwrap_or_else(|e| e.into_inner());
        let screen = parser.screen();
        let (rows, cols) = screen.size();

        let mut numbered_options: Vec<(usize, String)> = Vec::new();
        ...
        for row in 0..rows {
            ...
            // Strip leading selector arrow (❯) and whitespace for pattern matching.
            let stripped = trimmed.trim_start_matches('❯').trim();

            // Parse "N. Label" pattern: leading digits followed by "." then label.
            let digit_end = stripped.find(|c: char| !c.is_ascii_digit()).unwrap_or(0);
            if digit_end > 0 && stripped.as_bytes().get(digit_end) == Some(&b'.') {
                let num_str = &stripped[..digit_end];
                if let Ok(num) = num_str.parse::<usize>() {
                    // Use the original trimmed text for the label to preserve spacing.
                    let label = stripped[digit_end + 1..].trim().to_string();
                    if !label.is_empty() && num >= 1 {
                        log::debug!(
                            "extract_permission_choices: row {row} parsed option {num}: '{label}'"
                        );
                        numbered_options.push((num - 1, label)); // 0-based index
                    }
                }
            }
        ...
        if numbered_options.len() >= 2 {
            ...
            numbered_options.sort_by_key(|&(i, _)| i);
            log::debug!(
                "extract_permission_choices: found {} numbered options: {:?}",
                numbered_options.len(),
                numbered_options,
            );
            Some(PermissionChoices {
                kind: PermissionPromptKind::NumberedSelector,
                options: numbered_options,
            })
        } else if has_yn_prompt {
            Some(PermissionChoices {
                kind: PermissionPromptKind::YesNoPrompt,
                options: vec![
                    (0, "Yes".to_string()),
                    (1, "No".to_string()),
                ],
    ...
    /// Send keystrokes to select the option at `option_index` (0-based).
    ///
    /// For numbered selectors, sends each Down arrow separately with a
    /// small delay so the TUI selector has time to process each keypress.
    ///
    /// Returns `false` if no recognisable permission prompt is visible or
    /// the index is out of range.
    pub fn send_permission_selection(
        &mut self,
        idx: usize,
        option_index: usize,
    ) -> bool {
        let choices = match self.extract_permission_choices(idx) {
            Some(c) => c,
            None => return false,
        };

        match choices.kind {
            PermissionPromptKind::NumberedSelector => {
                if option_index >= choices.options.len() {
                    return false;
                }
                // The selector starts with the first option highlighted.
                // Press Down `option_index` times, then Enter.
                // Each keystroke is sent separately with a delay.
                const KEYSTROKE_DELAY: Duration = Duration::from_millis(50);
                for i in 0..option_index {
                    if self.write_to_session(idx, b"\x1b[B").is_err() {
                        return false;
                    }
                    if i < option_index - 1 {
                ...
                // Delay before Enter to let the last arrow key be processed.
                if option_index > 0 {
                    thread::sleep(KEYSTROKE_DELAY);
                }
                self.write_to_session(idx, b"\r").is_ok()
            }
            PermissionPromptKind::YesNoPrompt => {
                let data = match option_index {
                    0 => b"y\r".as_slice(),
                    1 => b"n\r".as_slice(),
                    _ => return false,
                };
                self.write_to_session(idx, data).is_ok()
            }
        }
    }

    /// Send keystrokes to approve or deny the permission prompt.
    ///
    /// For approve: selects index 0. For deny: selects the last option.
    /// Returns `false` if no recognisable permission prompt is visible.
    pub fn send_permission_response(&mut self, idx: usize, approve: bool) -> bool {
        let choices = match self.extract_permission_choices(idx) {
            Some(c) => c,
            None => return false,
        };
        let option_index = if approve {
            0
        } else {
            choices.options.len().saturating_sub(1)
        };
        log::debug!(
            "send_permission_response: approve={}, option_index={}, total_options={}",
            approve, option_index, choices.options.len(),
        );
        self.send_permission_selection(idx, option_index)
    }
