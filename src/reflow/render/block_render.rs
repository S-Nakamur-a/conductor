...
/// 実測: テキストブロックは独立したターンとして描かれ、間に空行が入る。区切りはエントリの
/// あいだにしか入らないので、この空行はここで入れる。本文は Markdown を通さない (生の入力)。
fn render_user_turn(
    pos: &BlockPos<'_>,
    styles: &TranscriptStyles,
    text: &str,
...
/// 折りたたみ時は 1 行で背景ブロック無し (› の字形が INACTIVE で全部担う)。思考ブロックと
/// 違い普通のチャット内容なので、本文は Markdown で描く。
#[allow(clippy::too_many_arguments)]
