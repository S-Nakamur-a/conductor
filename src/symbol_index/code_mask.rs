//! 行上のどの識別子の出現が *コード* の位置にあるか — コメントや文字列リテラルの中に
//! あるのとは対照的に。無いと、doc コメント内の英単語が同名のシンボルに解決される。
//!
//! # バイト範囲ではなく出現インデックスを使う理由
//!
//! viewer はタブを展開した行を保持しているので、ソース中のバイトオフセットは画面上の
//! カラムと一致しない (対象言語の 1 つである Go は慣習としてタブインデントされる)。
//! タブ展開は識別子の並び替えも書き換えもしないので、「この行の k 番目の識別子」は展開を
//! 生き延びる。両側が識別子とみなすものは一致していなければならず、そのため
//! [identifier_occurrences] が唯一の定義になっている。
//!
//! # allowlist にする理由
//!
//! 記録するのは、マスクされて除外された出現ではなく *コードである* 出現。allowlist が
//! 空なら何もジャンプできない (機能が黙って何もしなくなる) が、blocklist が空だと
//! すべてがジャンプ可能になり、まさにこのモジュールが直そうとしているバグが黙って再発する。
//! 起きないジャンプは何も起きなかったのと同じだが、間違った場所へのジャンプは読んでいた
//! ものを押しのける。文法を持たない言語のファイルも同時に片付く — マスクが与えられない
//! ので、その中では何もジャンプできない。
...
///
/// マスクはこの並びの中の位置をキーにしているので、両側が識別子とみなすものが食い違うと
/// インデックスが黙ってずれ、マスクは間違った単語について答える。
///
...
impl CodeMask {
    ...
    ///
    /// 「コードである識別子が 1 つもない」とは扱いが正反対なので区別する。ジャンプの提示は
    /// 1 つの単語についての主張なので沈黙が安全な答えだが、参照の一覧はリポジトリ全体に
    /// ついての主張なので「結果なし」は「存在しない」と主張することになる。検索に答える
    /// 呼び出し側は、保証できない空の結果を報告せずフィルタなしの一致に落ちるべき。
