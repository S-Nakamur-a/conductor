...
pub fn render_grep_search_overlay(frame: &mut Frame, area: Rect, app: &mut App) {
    ...
    let search_border_color = if input_focused {
    ...
    let case_indicator = if app.overlays.grep_search.case_sensitive {
        "[Aa]"
    } else {
        "[aa]"
    };
    ...
    if available_for_query > mode_width + 3 {
        let spans = vec![
            Span::styled(
                ...
                Style::default().fg(if app.overlays.grep_search.regex_mode {
                    theme.accent
                } else {
                    theme.muted
                }),
            ...
            Span::styled(
                ...
                Style::default().fg(if app.overlays.grep_search.case_sensitive {
                    theme.accent
                } else {
                    theme.muted
                }),
        ...
        if input_focused {
            ...
            if cursor_x < search_inner.x + search_inner.width
                && cursor_y < search_inner.y + search_inner.height
            {
                frame.set_cursor_position(Position::new(cursor_x, cursor_y));
            }
        }
    ...
    let status_text = if app.overlays.grep_search.running {
        ...
        let pos = if rows_count > 0 {
            app.overlays.grep_search.selected + 1
        } else {
            0
        };
        format!(
            "  {pos}/{rows_count} rows ({total_matches} matches)  |  Ctrl+R: regex  Ctrl+I: case"
        )
    };
    frame.render_widget(
            ...
            let line = match row {
                ...
                SearchTreeRow::Match { match_index, .. } => {
                    ...
                    if let Some(m) = matches.get(*match_index) {
                        ...
                        let max_content =
                            inner_width.saturating_sub(indent_str.len() + 2 + location.len() + 3);
                        ...
                        if ms < me && me <= content.len() && ms < safe_max {
                            ...
                            let safe_trunc =
                                floor_char_boundary(content, max_content.saturating_sub(3));
