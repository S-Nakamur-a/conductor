...
/// ブロック 1 つぶんの行を syntect にかける。
///
/// ハンクは飛び飛びの断片なので、パーサの状態はファイル先頭からの続きには
/// ならない (文字列やコメントの途中から始まると数行ぶん色がずれる)。それでも
/// 全部を無彩色にするよりは読める、という判断でブロック単位に割り切っている。
/// 削除行と追加行が同じ流れに混ざるのも同じ理由。
fn highlight_block(
    app: &App,
...
/// diff の 1 行。
///
/// 追加・削除は背景色で示す。+/- の記号を出さないのは、構文の色を前景に使う
/// 以上、前景で追加・削除を表せないため — 記号と背景で二重に言うと帯 (持ち物の
/// 印) と合わせて 3 つの印が並び、どれが何かが読めなくなる。
fn diff_line(
...
/// 単純な折り返し。単語境界は狙わない (本文は日本語が主なので効かない)。
///
/// 切るのは文字数ではなく表示幅。日本語は 1 文字が 2 列なので、文字数で切ると
/// 幅の 2 倍に伸び、はみ出した分は枠で黙って落ちる。
