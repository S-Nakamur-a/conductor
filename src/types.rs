//! アプリ全体で共有する語彙。
//!
//! ここに置くのは、どのサブシステムからも名前で参照される型だけに限る。
//! パネルのフォーカス、ステータス通知、worktree 操作の入力モードと結果など。
//! `app` に置いていた頃は、型を1つ借りたいだけの下位モジュールが `app` 全体に
//! 依存することになり、依存グラフに循環を作っていた。
//!
//! このモジュールは `keymap` 以外のクレート内モジュールを参照しない。
//! 参照が必要になった型は、ここではなく持ち主のモジュールへ置くこと。
