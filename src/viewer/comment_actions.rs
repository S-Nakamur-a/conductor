//! Viewer から起動するコメント操作: 現在位置をプレフィルした入力欄を開く、
//! 現在行のコメント詳細を開く、送信されたテキストを新しいコメントにする。
//!
//! 分割前は Explorer の入力ディレクトリに置かれていたが、Explorer の描画にも
//! 入力にも関与せず、呼び出し元は Viewer とレビューのオーバーレイだけだった。
