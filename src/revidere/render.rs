...
//!
//! 概要を別画面にしているのは、読むのが最初の一度きりだから。混ぜるとそのあとずっと縦を
//! 取り続ける (GitHub が PR の説明と Files changed を分けているのと同じ)。画面全体を
//! 占有するのは、狭いペインでは項目の説明と diff を同時に読むという唯一の用途が
//! 成立しないため。
//!
//! 右の列を歩くのは diff であって項目ではない
//!
//! 行を出しているのは [revidere::ReadingOrder] で、そのループの主語は変更一覧の側。
//! 項目が漏らしても変更行は消えず、最悪でも帯の無い素の diff に退化する。項目を回して
//! 「その項目が触るファイル」を出す形に書き直すと、その保証が失われる。
//!
//! 項目の先頭行は描画中に記録する
//!
//! n/N のジャンプ先は「その項目より前が出した表示行の総和」だが、本文の折り返しが幅に
//! 依存するので幅を知らない場所では数えられない。描画のたびに
//! [crate::app::RevidereState::section_rows] へ書き込み、キー処理はそれを読む。
//!
//! 右列はキャッシュする
//!
//! syntect のハイライトと折り返しは全変更行ぶんを毎フレームやり直すには重い。幅・
//! テーマ・成果物のどれかが変わったときだけ組み直す。
