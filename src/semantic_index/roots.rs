...
impl IndexRoot {
    ...
    /// 内容の違うツリーの索引を並べて持てる。
    ///
    /// 畳むのはこの言語のファイルだけ。全ファイルを畳むと、画像を差し替えただけで鍵が
    /// 変わって中身の同じ索引を作り直す。`deeper` (このルートの中の、同じ言語のより深い
    /// ルート) を除くのも同じ理由で、数えると内側の編集で外側の鍵まで動く。
    ///
    ...
    /// (相対パス, 内容ハッシュ) の表を鍵に畳む。入口が 2 つある (ツリーを歩いたものと、
    /// producer が書いた出自の表) ので、絞り込みの規則はここ 1 箇所に置く。ずれると、
    /// 生成した索引の名前と次に探す名前が食い違い、毎回作り直しになる。
    pub fn fold(
        &self,
    ...
    /// 古い世代を落とす。新しいほうから [`GENERATIONS`] 本だけ残す。名前に鍵が入る前に
    /// 置かれた索引も落とす — 誰も読まないのに 1 本 14.5MB 残り続けるため。消せなくても
    /// 何もしない。残るのはディスクの無駄だけで、答えは変わらない。
