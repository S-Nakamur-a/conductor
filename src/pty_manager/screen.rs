...
impl PtyManager {
    ...
    ///
    /// 生バイト履歴の再生でパーサが再構築された (新しい幅でリフローされた) 場合に true。
    ///
    /// vt100 の set_size はリフローしない — 列数の変更時にラップフラグをクリアして行を
    /// 切り詰め/パディングするだけなので、以前ラップされていた行は旧幅のままになる。
    /// raw_history を持つセッション (シェル) では記録済みの生バイトからパーサを再構築して
    /// 再ラップさせ、それ以外は set_size に落ちる。
    pub fn resize_session(&mut self, idx: usize, rows: u16, cols: u16) -> bool {
        // vt100::Parser::new は非ゼロの寸法を要求するため防御的にクランプする。
        let rows = rows.max(1);
        let cols = cols.max(1);
        ...
        // 実際の PTY をリサイズする (SIGWINCH を配送し、子プロセスがライブ領域を再描画する)。
