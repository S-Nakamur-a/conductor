// AI の応答を貯める。
//
// 1 回の抽出に数分かかる。表示を直す・充足検査をかけ直す・ホストを変えて眺める、
// といった作業は成果物さえあれば済むので、そこでモデルを起こしていたら試行が止まる。
//
// 鍵は「答えを決めているものすべて」。プロンプト（＝ base/head と台帳）だけでは
// 足りない。作業ツリーを見ているとき、同じ行を書き換えれば台帳は変わらないのに
...
mod tests {
    ...
    /// 台帳が同じでも中身が違えば別物。作業ツリーを見ているときに、同じ行を
