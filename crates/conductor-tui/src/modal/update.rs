...
#[derive(Debug)]
pub enum Update {
    Confirm(Box<UpdateInfo>, Choice),
    Running(&'static str),
    Failed(String),
}

impl Update {
    pub fn confirm(info: UpdateInfo) -> Self {
        Self::Confirm(
            Box::new(info),
            Choice::new(Answer::Yes).labels("Update", "Cancel"),
        )
    }

    pub fn update(&mut self, key: KeyEvent, _ctx: &Ctx) -> Vec<Effect> {
        match self {
            Self::Confirm(info, choice) => match choice.key(key) {
                Some(Answer::Yes) => {
...
pub fn title(modal: &Update) -> String {
    match modal {
        Update::Confirm(info, _) => format!("Update to v{}", info.latest_version),
...
pub fn lines(modal: &Update, ctx: &Ctx) -> Vec<Line<'static>> {
    ...
    match modal {
        Update::Confirm(info, choice) => vec![
            Line::styled(
                format!(
...
pub(crate) mod tests {
    ...
    fn yでダウンロードが始まりnは何もしない() {
        ...
        let mut modal = Update::confirm(info());
        assert!(matches!(
