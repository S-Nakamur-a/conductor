...
#[test]
fn 幅で折り返し単語は壊さない() {
    let lines = render_rich("the quick brown fox", 9);
    for line in &lines {
...
fn 全角のcjkは表示幅で折り返す() {
    ...
    let lines = render_rich("ああああああ", 10);
    for line in &lines {
...
#[test]
fn 長すぎるトークンはハード分割する() {
    let lines = render_rich(&"a".repeat(12), 5);
...
#[test]
fn 知らない言語でも落ちずに落ち着く() {
    let lines = render_rich("```brainfuck\n+++.\n```", 40);
    let joined: String = lines.iter().map(line_text).collect();
...
fn インラインのコードもカードの背景に載る() {
    ...
    let spans = inline_spans("run `cargo test` now", base, &theme, Flavor::Rich);
    let code = spans
        .iter()
...
fn transcriptのh1は太字斜体下線でh2とh3は太字だけ() {
    ...
    let rich_h1 = render_rich("# Title", 30);
...
fn transcriptのインラインコードは余白なしのinfo色() {
    ...
    // （影響を受けない）。
    let rich = render_rich("use `git` now", 40);
    assert!(
...
fn transcriptの引用は減光したグリフと既定色の斜体本文() {
    ...
    // （影響を受けない）。
    let rich = render_rich("> quoted text", 40);
