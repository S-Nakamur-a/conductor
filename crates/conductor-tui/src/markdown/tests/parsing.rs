...
fn snake_caseと素のアスタリスクはそのまま() {
    ...
    // アンダースコアは強調にならない。
    let spans = inline_spans("call set_change_summary here", base, &theme, Flavor::Rich);
    ...
    let spans = inline_spans("rate is 2 * 3", base, &theme, Flavor::Rich);
...
fn 太字と斜体とコードに装飾が付く() {
    ...
    let spans = inline_spans("a **b** c", base, &theme, Flavor::Rich);
    ...
    let spans = inline_spans("use `git` now", base, &theme, Flavor::Rich);
...
fn 閉じていないインラインの記号はそのまま() {
    ...
    for input in [
        ...
        let spans = inline_spans(input, base, &theme, Flavor::Rich);
...
fn 打ち消し線は装飾と減光が付く() {
    let (theme, _, _) = fixtures();
    let base = Style::default().fg(theme.fg);
    let spans = inline_spans("keep ~~drop~~ this", base, &theme, Flavor::Rich);
...
fn 打ち消し線の中の記法は入れ子にしない() {
    ...
    let (theme, _, _) = fixtures();
    let base = Style::default().fg(theme.fg);
    let spans = inline_spans("~~old **bold**~~ and **real**", base, &theme, Flavor::Rich);
    assert!(
        spans
            .iter()
...
fn チェックボックスは幅の中に収まる() {
    ...
    for width in [8usize, 20, 40] {
        for line in render_rich("- [x] done\n- [ ] あいうえお task", width) {
...
fn 表は幅に収め必要なら切り詰める() {
    ...
    for width in [0usize, 1, 2, 3, 8, 20, 80] {
        let lines = render_rich(table, width);
        for line in &lines {
            assert!(
...
fn 幅の広いセルは失わずに折り返す() {
    ...
    for width in [20usize, 30, 40, 60, 100] {
        let text: String = render_rich(table, width)
            .iter()
            .map(|l| line_text(l))
            .collect::<Vec<_>>()
            .join("\n");
...
fn 分割不能な長いセルは切らずに割る() {
    ...
    let text: String = render_rich(&table, 24)
        .iter()
        .map(|l| line_text(l))
        .collect::<Vec<_>>()
        .join("");
...
fn セルの切り詰めはマルチバイトを割らない() {
    ...
    for width in [1usize, 2, 3, 4, 5, 6, 10] {
        for line in render_rich(table, width) {
...
fn 揃え方を変えてもセルの幅は変わらない() {
    ...
    let mk = |delim: &str| render_rich(&format!("| h |\n| {delim} |\n| ab |"), 20);
...
fn 列数の揃わない行は正規化する() {
    ...
    // 切り詰めされる。
    let table = "| a | b |\n| - | - |\n| 1 |\n| 1 | 2 | 3 |";
    let lines = render_rich(table, 40);
    // ヘッダー + 区切り線 + 2行。
...
fn リンクは本文とurlの両方を出す() {
    ...
    let spans = inline_spans("[**bold** link](https://x.io)", base, &theme, Flavor::Rich);
    assert!(
        spans
            .iter()
...
fn 本文がurlと同じか空ならurlは一度だけ() {
    ...
    let spans = inline_spans("[https://x.com](https://x.com)", base, &theme, Flavor::Rich);
    ...
    let spans = inline_spans("[](https://x.com)", base, &theme, Flavor::Rich);
    assert_eq!(joined(&spans), "https://x.com");
}
